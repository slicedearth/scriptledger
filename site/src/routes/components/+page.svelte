<script lang="ts">
  ...
  import { demoReport } from '$lib/demo-report.js';
</script>
...
<div class="table-wrap" role="region" aria-label="Detected components" tabindex="0">
  <table><caption>Synthetic component identifications.</caption><thead><tr><th scope="col">Component</th><th scope="col">Version</th><th scope="col">Confidence</th><th scope="col">Method</th><th scope="col">Limitation</th></tr></thead>
    <tbody>{#each demoReport.capture.components as component}<tr><th scope="row">{component.component}</th><td><code>{component.version ?? 'Unknown'}</code></td><td><State value={component.confidence} /></td><td>{component.identificationMethod.replaceAll('_', ' ')}</td><td>{component.limitations.join(' ')}</td></tr>{/each}</tbody>
  </table>
</div>
